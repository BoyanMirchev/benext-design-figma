...
export default function ServicesPage(){return <main>
  <div className="services-hero-wrap"><Header overlay/><section className="services-hero"><div className="shell services-hero__inner">
    ...
    <div className="service-top-card">{serviceTop.map(({icon:Icon,title,text})=><div key={title}><div className="blue-circle"><Icon/></div><h3>{title}</h3><p>{text}</p></div>)}</div>
  </div></section></div>
  <section className="build-section section-shadow"><div className="shell"><h2 className="section-title centered">Какво можем да направим за Вас</h2><div className="build-grid">
  ...
  <section className="idea-section section-shadow"><div className="shell"><h2 className="section-title">Имате идея за Вашия уебсайт?</h2><p>Просто ни пишете или се обадете. Ще се радваме да научим<br/>повече за Вашите цели, и да Ви предложим идеи за<br/>реализацията им, както и подходяща цена.</p><Button href="/contacts">Свържете се с нас</Button></div></section>
  <section className="compare"><div className="shell"><h2 className="section-title centered">Сравнете възможностите</h2><div className="compare-tabs"><strong>Уеб дизайн</strong><span>eCommerce</span><span>Бизнес и<br/>Счетоводство</span><span>Реклами и<br/>Оптимизация</span></div>
...
function Build({img,title,text}:{img:string;title:string;text:string}){return <article className="build-card"><img src={img} alt=""/><h3>{title}</h3><p>{text}</p></article>}
